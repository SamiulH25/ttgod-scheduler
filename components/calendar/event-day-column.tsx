  ...
  return (
    ...
    >
      ...
      {proposalSegments.map(({ proposal, ...seg }) => (
        <CrayonBlock
          key={`proposal-${seg.eventId}-${seg.startMin}`}
          color={seg.color}
          variant="preview"
          className={cn(
            "pointer-events-none left-0.5 right-0.5 z-[2] border-dashed opacity-90",
            segmentPositionClass(seg.position),
          )}
          style={{
            top: `${seg.topPercent}%`,
      ...
      {selection && !dragRange && (
        <CrayonBlock
          ...
          color="var(--primary)"
          className={cn(
            "pointer-events-none left-0.5 right-0.5 z-[3] border-primary",
